...
#include <cstddef>
#include <glm/glm.hpp>
#include "../ubo_layout.h"

static_assert(sizeof(GlobalParamsUBO) % 16 == 0,
              "GlobalParamsUBO size must be a multiple of 16 bytes for std140 uniform buffers");
static_assert(offsetof(GlobalParamsUBO, enableGrid) % 4 == 0,
              "GlobalParamsUBO enableGrid offset must stay 4-byte aligned");
static_assert(offsetof(GlobalParamsUBO, rgbOverlay) % 16 == 0,
              "GlobalParamsUBO rgbOverlay must stay vec3-aligned for std140");
